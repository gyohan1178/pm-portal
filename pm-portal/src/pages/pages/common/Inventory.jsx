...
const JS_CAT = { CA:'케이블', WI:'와이어', CN:'커넥터', BK:'차단기', EL:'전장', SM:'판금', AS:'어셈블리', HS:'하네스', HW:'하드웨어', ET:'기타' }
const PROC_CATS = new Set(['어셈블리','하네스','판금'])
const catOf = item => {
  const m = (item?.js_code || '').match(/^JS-?([A-Za-z]{2})/i)
  return (m && JS_CAT[m[1].toUpperCase()]) || item?.type || '-'
}
...
export default function Inventory() {
  ...
  const zeroMut = useMutation({
    ...
    onError: (e) => alert('변경 오류: ' + e.message),
  })
  ...
  function downloadAuditTemplate() {
    if (!rows.length) { alert('대상 품목이 없습니다.'); return }
    const data=rows.map(r=>({'기준코드':r.items?.std_code,'품명':r.items?.name,'제조사':r.items?.manufacturer||'','제조사품번':r.items?.manufacturer_code||'','단위':r.items?.unit,'현재고(참고)':r.qty,'실사수량':''}))
    const wb=XLSX.utils.book_new()
